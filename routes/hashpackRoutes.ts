...
export function setupHashPackRoutes(app: Express) {
  // Create patent hash transaction for HashPack signing
  app.post('/api/hashpack/patent-hash-transaction', requireAuth, async (req: any, res) => {
    try {
      ...
      const result = await hashPackHederaService.createPatentHashTransaction(
        patentId,
        filePath,
        hashPackWallet.accountId,
        hashPackWallet.network
      );
  ...
  app.post('/api/hashpack/patent-message-transaction', requireAuth, async (req: any, res) => {
    try {
      ...
      const result = await hashPackHederaService.createPatentMessageTransaction(
        patentId,
        hash,
        topicId,
        hashPackWallet.accountId,
  ...
  app.post('/api/hashpack/nft-transaction', requireAuth, async (req: any, res) => {
    try {
      ...
      // Create NFT transaction for HashPack signing
      const result = await hashPackHederaService.createPatentNFTTransaction(
        patent,
        hashPackWallet.accountId,
        hashPackWallet.network
      );
  ...
  app.post('/api/hashpack/nft-mint-transaction', requireAuth, async (req: any, res) => {
    try {
      ...
      const result = await hashPackHederaService.createNFTMintTransaction(
        tokenId,
        metadata,
        hashPackWallet.accountId,
        hashPackWallet.network
  ...
  app.post('/api/hashpack/submit-transaction', requireAuth, async (req: any, res) => {
    try {
      ...
      const result = await hashPackHederaService.submitSignedTransaction(
