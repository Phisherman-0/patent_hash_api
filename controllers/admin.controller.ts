...
export const adminController = {
  ...
  updateUserRole: async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { role } = req.body;
      if (!['user', 'consultant', 'admin'].includes(role)) return res.status(400).json({ message: 'Invalid role' });
  ...
  getAllConsultants: async (req: Request, res: Response) => {
    try {
  ...
  getUnverifiedConsultants: async (req: Request, res: Response) => {
    try {
  ...
  verifyConsultant: async (req: Request, res: Response) => {
    try {
      if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
      const { id } = req.params;
      const { notes } = req.body;
      const verifiedConsultant = await storage.verifyConsultant(id, req.user.id, notes);
      if (!verifiedConsultant) return res.status(404).json({ message: 'Consultant not found' });
  ...
  rejectConsultant: async (req: Request, res: Response) => {
    try {
      if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
      const { id } = req.params;
      const { notes } = req.body;
      const rejectedConsultant = await storage.rejectConsultant(id, req.user.id, notes);
      if (!rejectedConsultant) return res.status(404).json({ message: 'Consultant not found' });
  ...
  getConsultantStatus: async (req: Request, res: Response) => {
    try {
