...
export const blockchainService = {
  ...
  getContract: (readOnly = true) => {
    return new ethers.Contract(
      CONTRACT_ADDRESS,
      PatentSignABI,
      readOnly ? blockchainService.getProvider() : blockchainService.getWallet()
    );
  },
   ...
   */
  isDocumentRegistered: async (hashValue: string): Promise<boolean> => {
    try {
   ...
   */
  registerDocument: async (hashValue: string): Promise<string | null> => {
    try {
   ...
   */
  getSigners: async (hashValue: string): Promise<string[]> => {
    try {
      const contract = blockchainService.getContract(true);
   ...
   */
  hasSigned: async (hashValue: string, signerAddress: string): Promise<boolean> => {
    try {
      const contract = blockchainService.getContract(true);
