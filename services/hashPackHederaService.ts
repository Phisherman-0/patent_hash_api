...
export interface HashPackTransactionRequest {
  ...
  signedTransactionBytes?: string; // Base64 encoded signed transaction bytes from HashPack
}
...
class HashPackHederaService {
   ...
   */
  async createPatentHashTransaction(patentId: string, filePath: string, accountId: string, network: 'testnet' | 'mainnet'): Promise<{
   ...
   */
  async submitSignedTransaction(signedTransactionBytes: string, network: 'testnet' | 'mainnet'): Promise<{
   ...
   */
  async storePatentHashWithHashPack(
    patentId: string, 
    filePath: string, 
    accountId: string, 
    network: 'testnet' | 'mainnet'
    ...
    try {
      ...
      const { transactionBytes, hash } = await this.createPatentHashTransaction(
        patentId, 
        filePath, 
        accountId, 
        network
   ...
   */
  async verifyPatentHash(topicId: string, messageId: string, expectedHash: string, network: 'testnet' | 'mainnet'): Promise<{
    verified: boolean;
    actualHash?: string;
    timestamp?: string;
    message: string;
