...
var REPLIT_AUTH_ENABLED = process.env.USE_REPLIT_AUTH === "true";
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/auth/user/settings", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.put("/api/auth/user/settings", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.get("/api/dashboard/category-stats", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app2.get("/api/patents", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app2.get("/api/patents/:id", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.post("/api/patents", isAuthenticated, upload.array("documents"), async (req, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app2.put("/api/patents/:id", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user.claims.sub;
      const patent = await storage.getPatent(req.params.id);
      if (!patent) {
  ...
  app2.delete("/api/patents/:id", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user.claims.sub;
      const patent = await storage.getPatent(req.params.id);
      if (!patent) {
  ...
  app2.post("/api/ai/prior-art-search", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.post("/api/ai/patent-valuation", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.post("/api/ai/similarity-detection", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.post("/api/ai/patent-drafting", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.get("/api/blockchain/verify/:patentId", isAuthenticated, async (req, res) => {
    try {
  ...
  app2.post("/api/blockchain/mint-nft/:patentId", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user.claims.sub;
      const patent = await storage.getPatent(req.params.patentId);
      if (!patent || patent.userId !== userId) {
  ...
  app2.get("/api/search/patents", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user.claims.sub;
